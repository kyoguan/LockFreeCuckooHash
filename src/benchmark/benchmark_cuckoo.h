#ifndef BENCHMARK_CUCKOO
#define BENCHMARK_CUCKOO
...
class BenchmarkCuckoo
{
  public:
    BenchmarkCuckoo(int op_count, int capacity, 
...
BenchmarkCuckoo::BenchmarkCuckoo(int op_count, int capacity, 
                                       int rweight, int idweight,
                                       int thread_count, double load_factor)
{
  std::cout << "*** BENCHMARKING CUCKOO MAP ***" << std::endl;
  m_op_count     = op_count;
...
void BenchmarkCuckoo::benchmark_correctness()
{
  bool correct = true;

  cuckoo_hash_table ht(2 * C_NUM_ELEMS, m_thread_count);
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    pthread_create(&workers[i], NULL, thread_insert<cuckoo_hash_table>, (void*)&args[i]);
  }
...
void BenchmarkCuckoo::benchmark_hp()
{
  cuckoo_hash_table ht(400000, m_thread_count);
  ...
  for (int i = 0; i < m_thread_count; i++)
  {
    ...
    pthread_create(&workers[i], NULL, thread_remove<cuckoo_hash_table>, (void*)&args[i]);
  }
...
void BenchmarkCuckoo::benchmark_all()
{
    cuckoo_hash_table ht(m_capacity, m_thread_count);
    ...
    for (int iter = 0; iter < NUM_ITERS; iter++)
    {
      ...
      for (int i = 0; i < m_thread_count; i++)
      {
        ...
        args[i].ht_p      = (void*)&ht;
        args[i].tid       = i;
        pthread_create(&workers[i], NULL, thread_service<cuckoo_hash_table>, (void*)&args[i]);
      }
    ...
    for (int iter = 0; iter < NUM_ITERS; iter++)
    {
      ...
      for (int i = 0; i < m_thread_count; i++)
      {
        ...
        args[i].elems     = keys;
        args[i].start     = i * num_elems;
        pthread_create(&workers[i], NULL, thread_service_low_contention<cuckoo_hash_table>, (void*)&args[i]);
      }
    ...
    for (int iter = 0; iter < NUM_ITERS; iter++)
    {
      ...
      for (int i = 0; i < m_thread_count; i++)
      {
        ...
        args[i].tid       = i;
        ht.insert(0, 0, 0);
        pthread_create(&workers[i], NULL, thread_service_high_contention<cuckoo_hash_table>, (void*)&args[i]);
      }
